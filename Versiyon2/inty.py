...
import time
from mcp_ops import load_mcp_servers
import json
...
def create_response_popup(response_text):
    """Sunucu yanıtını ve MCP araç seçimini gösteren modern popup."""
    ...
    active_popup.geometry("700x850")
    active_popup.configure(bg="#f5f5f5")
    active_popup.resizable(True, True)

    main_frame = tk.Frame(active_popup, bg="#f5f5f5")
    main_frame.pack(expand=True, fill="both", padx=20, pady=15)

    # ... (Font kontrolü, başlık, yanıt metni gibi kısımlar değişmez) ...
    # Bu kısımları kodunuzdaki gibi bırakın.
    # Örnek olarak yanıt etiketi:
    control_frame = tk.Frame(main_frame, bg="#f5f5f5")
    control_frame.pack(fill="x", pady=(0, 10))
    # ... (Diğer widget'lar) ...
    html_content = markdown.markdown(response_text)
    initial_html = f'<div style="font-family: Arial; font-size: {current_font_size}px;">{html_content}</div>'
    response_label = HTMLLabel(main_frame, html=initial_html, padx=10, pady=10)
    ...
    tk.Label(
        question_frame, text="Ask a new question:", font=("Arial", 12, "bold"),
        bg="#f5f5f5", fg="#333333"
    ).pack(anchor="w")

    new_question_entry = tk.Entry(
        question_frame, font=("Arial", 11), relief="flat", bg="white",
        highlightthickness=1, highlightbackground="#dddddd", highlightcolor="#007bff"
    )
    new_question_entry.pack(fill="x", pady=(5, 10), ipady=8)

    # --- MCP Araç Seçim Arayüzü (Bu bölüm değişmeden kalır ve doğru çalışır) ---
    mcp_frame = tk.Frame(question_frame, bg="#f5f5f5")
    mcp_frame.pack(fill="x", pady=(5, 15))

    tk.Label(
        mcp_frame, text="Select a Tool (Optional):", font=("Arial", 10, "italic"),
        bg="#f5f5f5", fg="#555555"
    ).pack(anchor="w", pady=(0, 5))

    # Radiobutton'ların seçimini tutacak değişken
    selected_mcp = tk.StringVar(value=None) 
    
    # Güncellenmiş fonksiyon çağrılır ve ['playwright', 'airbnb'] listesini döndürür
    mcp_servers = load_mcp_servers()
    
    # ['playwright', 'airbnb'] listesindeki her eleman için bir Radiobutton oluşturulur
    for server_name in mcp_servers:
        rb = ttk.Radiobutton(
            mcp_frame,
            text=server_name,
            variable=selected_mcp,
            value=server_name
        )
        rb.pack(side="left", padx=5, anchor="w")
    # -------------------------------------------------------------------
    ...
    # send_new_question fonksiyonu da değişmeden doğru çalışır
    def send_new_question(event=None):
        question = new_question_entry.get().strip()
        mcp_choice = selected_mcp.get()

        if mcp_choice == "None" or not mcp_choice: # Daha sağlam bir kontrol
            mcp_choice = None
            
        if question:
            active_popup.withdraw()
            # gem_ops fonksiyonuna doğru MCP adı (örn: "playwright") veya None gönderilir
            new_answer = gem_ops.generate_text_content(question, desired_mcp_name=mcp_choice)
            create_response_popup(new_answer)
        else:
            messagebox.showwarning("Warning", "Please enter a question.")

    ask_button = tk.Button(
        button_frame, text="Ask", command=send_new_question, bg="#007bff",
        fg="white", font=("Arial", 11, "bold"), relief="flat", padx=20, pady=8
    )
    ask_button.pack(side="left", padx=(0, 10))

    close_button = tk.Button(
        button_frame, text="Close", command=active_popup.destroy, bg="#dc3545",
        fg="white", font=("Arial", 11, "bold"), relief="flat", padx=20, pady=8
    )
